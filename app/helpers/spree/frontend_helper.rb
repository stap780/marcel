module Spree
    module FrontendHelper
      ...
      def logo(image_path = nil, options = {})
        ...
        link_to path, 'aria-label': current_store.name, class: '', method: options[:method] do
          image_tag image_path, alt: current_store.name, title: current_store.name, class: 'w-100'
        end
      end
      ...
      def flash_messages(opts = {})
        ...
        flash.to_h.except('order_completed').each do |msg_type, text|
          ...
          flashes << content_tag(:div, class: "alert alert-#{class_for(msg_type)} mb-0 d-flex justify-content-center align-items-center") do
            content_tag(:button, '&times;'.html_safe, class: 'btn close', data: { dismiss: 'alert', hidden: true }) +
