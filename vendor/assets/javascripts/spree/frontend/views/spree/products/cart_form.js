...
function CartForm($, $cartForm) {
  ...
  this.initialize = function() {
    ...
    this.withColorRelated = $cartForm.attr('data-product-with-color-related')
    ...
    this.$sku = $cartForm.find('#product_variant_sku')
  ...
  this.initializeForm = function() {
    if (this.withOptionValues) {
      ...
      //   })
      // }
      if (this.withColorRelated){
        var initOptionValues = []
        this.optionTypes().each(function(_, optionType) {
          var $optionValues = $(optionType).find(OPTION_VALUE_SELECTOR)
          initOptionValues.push($optionValues.first())
        })

        initOptionValues.forEach(function($value) {
          this.applyCheckedOptionValue($value, true)
        })
      }
    } else {
  ...
  this.showAvailableVariants = function() {
    ...
    console.log('availableOptionValueIds',availableOptionValueIds)
    var selectedOptionValueIdsCount = this.selectedOptionValueIds.length
    console.log('selectedOptionValueIdsCount',selectedOptionValueIdsCount)

    this.optionTypes().each(function(index, optionType) {
      console.log('index',index)
      if (index < selectedOptionValueIdsCount) return

      $(optionType).find(OPTION_VALUE_SELECTOR).each(function(_index, ov) {
          var $ov = $(ov)
  ...
  this.updateVariantSku = function() {
    var variant = this.selectedVariant()
    var variantSku = (variant && variant.sku) || ''

    this.$sku.html(variantSku)
  }
